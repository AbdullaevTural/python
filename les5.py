...
text = open("Ice.txt", encoding="utf-8")
string = text.readlines()
text.close()
a = set (string[0].replace('\n', '').split(', '))
b = set (string[1].replace('\n', '').split(', '))
s = a.difference(b)
print('Закончилось:', s)
...
# Задача 4*. Даны два файла, в каждом из которых находится запись многочлена. Найдите сумму данных многочленов. (Загуглил, крутое решение особенно с созданием этого много члена)
import random
